...
const char* settings = "//Place your settings in this file to overwrite default and user settings.FALSE\r\n\
{\r\n\
    \"javascript.validate.enable\": false,\r\n\
    \"typescript.tsdk\" : \"";
const char* settingspost = "\"\r\n\
}";
...
static void init_project() {
    ...
    fp = fopen(".vscode/tsconfig.json", "wb");
    ...
    fp = fopen(".vscode/settings.json", "wb");
    fwrite(settings, 1, strlen(settings), fp);
    const char* ijjs = getenv("IJJS");
    if (ijjs)
    {
        fwrite(ijjs, 1, strlen(ijjs), fp);
        fwrite(settingspost, 1, strlen(settingspost), fp);
    }
    else
        goto initerror;
    fclose(fp);
