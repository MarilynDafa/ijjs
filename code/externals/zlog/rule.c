 ...
 * return NULL	fail
 */
#define zlog_rule_gen_path(a_rule, a_thread)                   \
	do                                                         \
	{                                                          \
		int i;                                                 \
		zlog_spec_t *a_spec;                                   \
...
static int syslog_facility_atoi(char *facility)
{
	...
	if (STRICMP(facility, ==, "LOG_USER"))
		return LOG_USER;
	if (STRICMP(facility, ==, "LOG_AUTHPRIV"))
		return LOG_AUTHPRIV;
	if (STRICMP(facility, ==, "LOG_CRON"))
		return LOG_CRON;
	if (STRICMP(facility, ==, "LOG_DAEMON"))
		return LOG_DAEMON;
	if (STRICMP(facility, ==, "LOG_FTP"))
		return LOG_FTP;
	if (STRICMP(facility, ==, "LOG_KERN"))
		return LOG_KERN;
	if (STRICMP(facility, ==, "LOG_LPR"))
		return LOG_LPR;
	if (STRICMP(facility, ==, "LOG_MAIL"))
		return LOG_MAIL;
	if (STRICMP(facility, ==, "LOG_NEWS"))
		return LOG_NEWS;
...
int zlog_rule_set_record(zlog_rule_t *a_rule, zc_hashtable_t *records)
{
	...
	if (a_rule->output != zlog_rule_output_static_record && a_rule->output != zlog_rule_output_dynamic_record)
	{
