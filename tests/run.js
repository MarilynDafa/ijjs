...
class Test {
    ...
    run() {
        const args = [ ijjs.exepath(), this._fileName ];
        this._proc = ijjs.spawn(args, { stdout: 'pipe', stderr: 'pipe' });
        this._stdout = this._slurpStdio(this._proc.stdout);
        this._stderr = this._slurpStdio(this._proc.stderr);
        this._timer = setTimeout(() => {
            this._proc.kill(ijjs.signal.SIGKILL);
            this._timeout = true;
        }, TIMEOUT);
...
(async function() {
    const dir = await ijjs.fs.realpath('D:/ijjs/tests');
    console.log(dir)
    const dirIter = await ijjs.fs.readdir(dir);
    const tests = [];
